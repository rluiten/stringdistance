    ...
    in
      describe "StringDistance tests"
        ...
        , describe "sift3Distance tests"
            ...
            , test "Two different strings \"abc\" \"ab\"" <| \() -> (Expect.equal 0.5 (sift3Distance "abc" "ab"))
            , test "Two different strings \"helloworld\" \"world\"" <| \() -> (Expect.equal 2.5 (sift3Distance "helloworld" "world"))

            -- These are to slow using lcs, fine with lcsLimit
            , test "1 char different start 17 chars long" <| \() -> (Expect.equal 0.5 (sift3Distance "abcdefghijklmnopq" "bcdefghijklmnopq"))
            , test "2 char different start 17 chars long" <| \() -> (Expect.equal 1.0 (sift3Distance "abcdefghijklmnopq" "cdefghijklmnopq"))
            , test "3 char different start 17 chars long" <| \() -> (Expect.equal 1.5 (sift3Distance "abcdefghijklmnopq" "defghijklmnopq"))
            , test "4 char different start 17 chars long" <| \() -> (Expect.equal 2.0 (sift3Distance "abcdefghijklmnopq" "efghijklmnopq"))
            , test "5 char different start 17 chars long" <| \() -> (Expect.equal 2.5 (sift3Distance "abcdefghijklmnopq" "fghijklmnopq"))
           ...
           -- this correlates with a test in Mailcheck for suggest to check string distance for its domains
            , test "check distance on this case" <| \() -> (Expect.equal 7.5 (sift3Distance "kicksend.com" "emaildomain.com"))
            ]
        , describe "lcs tests"
            [ test
